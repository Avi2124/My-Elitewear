...
const Hero = () => {
  ...
  const desktopSlides = [
    banners.desktop.at(-1),
    ...banners.desktop,
    banners.desktop[0],
  ];
  const items = useMemo(
    () => (mobile ? banners.mobile : desktopSlides),
    [mobile],
  );
  ...
  useEffect(() => {
    if (!mobile) {
      ...
      if (index === 0) {
        const t = setTimeout(() => {
          setTransitioning(false);
          setIndex(desktopSlides.length - 2);
        }, 1000);
        return () => clearTimeout(t);
      }
    ...
    >
      ...
      <div className="absolute bottom-8 left-4 md:left-8 z-10 text-white">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="uppercase tracking-widest text-xs font-medium text-white/80">
            New Arrivals
          </p>
        </div>
